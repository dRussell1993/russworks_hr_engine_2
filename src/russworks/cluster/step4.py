...
_PARK_FACTOR_GRADE_RANK = {"Elite": 5, "Strong": 4, "Moderate": 3, "Neutral": 2, "Suppressive": 1}
...
class Step4ClusterEngine:
    ...
    def rank_cluster_batters(self, team: str, batter_reviews: Sequence[BatterReview]) -> TeamClusterReport:
        ...
        park_factor_score = max(review.park_factor_score for review in reviews)
        park_factor_confidence = mean(review.park_factor_confidence for review in reviews)
        park_factor_grade = _top_park_factor_grade(reviews)
        ...
        park_factor = [
            review
            for review in ranked_batters
            if review.park_factor_grade in {"Elite", "Strong", "Moderate"}
        ]
        ...
        return TeamClusterReport(
            ...
            park_factor_bats=[review.batter_name for review in park_factor],
...
def _cluster_score(
    ...
    max_park_factor = max((review.park_factor_score for review in reviews), default=0.0)
    special_depth = min(
        8.0,
        sum(
            1.5
            ...
            or review.park_factor_grade in {"Elite", "Strong", "Moderate"}
        ),
    )
...
def _batter_cluster_score(review: BatterReview) -> float:
    ...
    score += review.park_factor_score * 0.08
    ...
    score += 2.0 if review.park_factor_grade in {"Elite", "Strong", "Moderate"} else 0.0
...
def _top_park_factor_grade(reviews: Sequence[BatterReview]) -> str:
    return max((review.park_factor_grade for review in reviews), key=lambda grade: _PARK_FACTOR_GRADE_RANK.get(grade, 0), default="Neutral")
...
def _cluster_notes(
    ...
    if any(review.park_factor_grade in {"Elite", "Strong", "Moderate"} for review in reviews):
        notes.append("Park Factor bats identified")
